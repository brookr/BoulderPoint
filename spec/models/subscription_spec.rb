...
describe Subscription do
  ...
  describe "" do
    ...
    describe "when destroyed" do
      before(:each) do
        @sub.stubs(:gateway).returns(@gw = ActiveMerchant::Billing::Base.gateway(:bogus).new)
        @sub.stubs(:paypal).returns(@pp = ActiveMerchant::Billing::Base.gateway(:bogus).new)
      end
    ...
    describe "when starting paypal" do
      before(:each) do
        @gw = ActiveMerchant::Billing::Base.gateway(:paypal_express_recurring).new(:login => 'login', :password => 'password', :signature => 'sig')
        @response = PaypalExpressResponse.new(true, 'Forced success', { :token => 'start' })
        ActiveMerchant::Billing::Base.gateway(:paypal_express_recurring).expects(:new).returns(@gw)
      end

      it "should set up the authorization redirect to start the process" do
        @gw.expects(:setup_agreement).with(:return_url => 'foo', :cancel_return_url => 'bar', :description => Saas::Config.app_name).returns(@response)
    ...
    describe "when completing paypal" do
      before(:each) do
        ...
        @gw = ActiveMerchant::Billing::Base.gateway(:paypal_express_recurring).new(:login => 'login', :password => 'password', :signature => 'sig')
        @bt = ActiveMerchant::Billing::Base.gateway(:braintree).new(:login => 'login', :password => 'password')
      ...
      it "should update the billing info with a successful agreement creation" do
        ...
        @sub.next_renewal_at.to_i.should == (@start_date + 1.day).to_i
      end

      it "should clear the card storage when switching from cc" do
        ActiveMerchant::Billing::Base.gateway(Saas::Config.gateway).expects(:new).returns(@bt)
...
def paypal_profile_options(opts = {})
  { :description => Saas::Config.app_name, :start_date => @start_date, :frequency => 1, :amount => @sub.amount_in_pennies, :currency => 'USD' }.merge(opts)
end
